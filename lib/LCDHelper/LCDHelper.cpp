...
void LCDHelper::printNext(const char *row1, const char *row2, const int color,
...
void LCDHelper::printNextP(const char *row1P, const char *row2P,
                           const int color, const unsigned long displayUntil) {
...
void LCDHelper::maybePrintNext(const char *row1, const char *row2,
                               const int color) {
...
void LCDHelper::maybePrintNextP(const char *row1P, const char *row2P,
                                const int color) {
  char row1[strlen_P(row1P) + 1];
  char row2[strlen_P(row2P) + 1];
  strcpy_P(row1, row1P);
...
void LCDHelper::maybeScrollRow(char result[LCD_ROW_STR_LENGTH],
                               const char *row) {
...
void LCDHelper::copy16(char dest[LCD_ROW_STR_LENGTH], const char *src) {
...
void LCDHelper::screenOn() { this->lcd->display(); }